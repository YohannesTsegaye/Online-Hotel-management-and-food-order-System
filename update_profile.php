...
function ValidateEmail() 
{
    ...
    var aa=document.forms["fuaa"]["email"].value;
  
    let dd=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
  if(!(aa.match(dd))){
alert("incorrect email formt");
...
function ValidateEmail() 
{
    ...
    var aa=document.forms["fuaa"]["phon"].value;
  
    let xy=/^\d{10}$/;
  if(!(aa.match(xy))){
alert("phone must be 10 Digets");
...
<section class="form-container update-form">
   ...
   <form action="" method="post" name="fuaa" onsubmit='ValidateEmail()' >
      <h3>update profile</h3>
      <input type="text" name="name" placeholder="<?= $fetch_profile['name']; ?>" class="box" maxlength="50">
      <input type="email" name="email" id="email" placeholder="<?= $fetch_profile['email']; ?>" class="box" maxlength="50" oninput="this.value = this.value.replace(/\s/g, '')">
      <input type="number" name="number" id="phon" placeholder="<?= $fetch_profile['number']; ?>" class="box" min="0" max="9999999999" maxlength="10">
