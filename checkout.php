...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      if($address == ''){
         ...
         $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, number, email, method, address, total_products, total_price,verno) VALUES(?,?,?,?,?,?,?,?,?)");
         $insert_order->execute([$user_id, $name, $number, $email, $method, $address, $total_products, $total_price ,$myrandom]);

         $delete_cart = $conn->prepare("DELETE FROM `cart` WHERE user_id = ?");
         $delete_cart->execute([$user_id]);

         $message[] = 'Order placed successfully and your confirmation code is!';

         header('location: generator.php?q= '.$myrandom.'');
      }
